...
def medico_signup_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and medicoForm.is_valid():
            ...
            medico.user = userForm
            especialidad, created = Especialidad.objects.get_or_create(nombre='A completar')
            medico.especialidad = especialidad
            medico.save()
            # Grupo de usuario
            my_medico_group = Group.objects.get_or_create(name='MEDICO')
            my_medico_group[0].user_set.add(userForm)
    ...
    if request.user.is_staff:
        ...
        if request.method=='POST':
            ...
            if userform.is_valid() and medicoform.is_valid():
                ...
                medico_group[0].user_set.add(userform)
